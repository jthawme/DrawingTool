...
class JetPlot {
    ...
    arc(cx, cy, radius, angle, segments = 50, startingAngle = 0) {
        this._checkPrevious();
        const path = getArc(cx, cy, radius, angle, segments, startingAngle);
    ...
    rect(tlx, tly, width, height) {
        this._checkPrevious();
        const path = getRect(tlx, tly, width, height);
        this._connectPath(path);
    ...
    cubicBezierTo(cp1x, cp1y, cp2x, cp2y, p2x, p2y, segments = 50) {
        ...
        const path = getCubicBezier(p1.x, p1.y, cp1x, cp1y, cp2x, cp2y, p2x, p2y, segments);
    ...
    quadraticBezierTo(cp1x, cp1y, p2x, p2y, segments = 50) {
        ...
        const p1 = this._shape.getLastPoint() || { x: 0, y: 0 };
        const path = getQuadraticBezier(p1.x, p1.y, cp1x, cp1y, p2x, p2y, segments);
